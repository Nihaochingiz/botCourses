...
const keyWords = ["фт","финтетрис","ФТ","финансовый тетрис"];
let step = 0;
...
function greeting(){
 ...
 if (/^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}$/.test(ctx.message.text) === false) {
  ...
  (ctx) => {
    ...
    ctx.scene.leave();
    ctx.reply(`До скорой встречи ${ctx.session.userName}, с вами свяжется наш специалист`);
  ...
  async function gsrun(cl) {
    ...
    const arrUserData = [ctx.session.userName,ctx.session.userPhone]
  
    const dataArray = {values:arrUserData }
